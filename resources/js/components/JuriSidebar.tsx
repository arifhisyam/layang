...
const SIDEBAR_STYLES = `
  ...
  .js-mobile-drawer {
    ...
    background: rgba(14,100,180,0.96);
    backdrop-filter: blur(32px) saturate(180%);
    -webkit-backdrop-filter: blur(32px) saturate(180%);
    border-right: 1px solid rgba(255,255,255,0.1);
  ...
  .js-mobile-topbar {
    ...
    background: rgba(14,100,180,0.92);
  ...
  @media (max-width: 768px) {
    .js-sidebar { display: none !important; }
    .js-mobile-topbar { display: flex; }
    .js-mobile-overlay.open { display: block; }
    .js-mobile-drawer.open { display: flex; }
  }
`;
...
export default function JuriSidebar({ user, activePage = 'overview', belumDinilai = 0 }: Props) {
                ...
                {navItems.map(item => {
                    ...
                    return (
                        ...
                        >
                            ...
                            {isCollapsed && <span className="js-tooltip">{item.label}</span>}
                        </Link>
                    );
                })}
            </nav>
    ...
    return (
        <>
            ...
            {/* ══ MOBILE TOP BAR ══ */}
            <div className="js-mobile-topbar">
                <div className="js-hamburger" onClick={() => setMobileOpen(o => !o)}>
                    {mobileOpen
            ...
            {/* ══ MOBILE OVERLAY ══ */}
            <div
                className={`js-mobile-overlay${mobileOpen ? ' open' : ''}`}
                onClick={() => setMobileOpen(false)}
            />

            {/* ══ MOBILE DRAWER ══ */}
            <div className={`js-mobile-drawer${mobileOpen ? ' open' : ''}`}>
                {sidebarContent(true)}
            </div>
        </>
    );
